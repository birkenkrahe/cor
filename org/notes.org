...
- *Remember: Spragin's Lunch in Spragin's house today 12-1 pm*
...
*** Question 2
Modify the following code to create a countdown from 5 to 0 using a
`while` loop.
...
*** Question 3
In the following code, what happens if the input is `'q'`?
...
#+begin_quote
If the input is `'q'`, the loop will break, stopping further
execution. If any other input is provided, it will print `"Done!"` and
then prompt for input again.
#+end_quote

*** Question 4
Write a Python program that checks if a number is smaller than 50,
greater than 50, or exactly 50, and prints an appropriate message.

#+begin_src python :session *Python* :results output
  foo = 50
  if foo < 50:
      print(f"{foo} is smaller than 50")
  elif foo > 50:
      print(f"{foo} is greater than 50")
  else:
      print(f"{foo} is exactly 50")
#+end_src

*** Question 5
How would you test if a number is either smaller than 10, between 10
and 50, or greater than 50? Provide the solution.

#+begin_src python :session *Python* :results output
  foo = 30
  if foo < 10:
      print(f"{foo} is smaller than 10")
  elif 10 <= foo <= 50:
      print(f"{foo} is between 10 and 50")
  else:
      print(f"{foo} is greater than 50")
#+end_src
